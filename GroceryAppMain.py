...
def GroceryApp():
  ...
  while True:
    ...
    if start_input == menu_keys["Make new list"]:
      new_list_menu(menu_divider, [menu_keys["Add to list"], menu_keys["Remove from list"]], list_item_divider, grocery_lists)
      ...
      for index in range(0, len(grocery_lists)):
        if grocery_lists[index].list_name == wanted_list:
          edit_list(grocery_lists[index], [menu_keys["Add to list"], menu_keys["Remove from list"]], list_item_divider)
    elif start_input == menu_keys["Display list"]:
      for list in grocery_lists:
        print(menu_divider)
        display_list(list, list_item_divider)
    elif start_input == menu_keys["Delete list"]:
      delete_list(grocery_lists, menu_divider, exit_keys)
